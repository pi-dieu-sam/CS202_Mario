...
public:
    ...
    [[nodiscard]] const sf::View& getUiView() const;
    [[nodiscard]] sf::Vector2f mapPixelToUiCoords(const sf::Vector2i& pixel) const;

    /// Persisted display/audio preferences.
    [[nodiscard]] GameSettings& getSettings();
    ...
    [[nodiscard]] std::vector<sf::VideoMode> getFullscreenModes() const;

    /// Access the state manager.
    [[nodiscard]] StateManager& getStateManager();

    /// Access player-progression data (score, lives, coins, level, character).
    [[nodiscard]] PlayerProgress& getProgress();
